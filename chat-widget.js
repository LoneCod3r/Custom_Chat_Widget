...
(function () {
  ...
  var CSS_TEXT = "\n"
    // The bubble's layout-critical properties carry !important: it's the one
    // element a customer's host CSS is statistically most likely to nuke by
    // accident, via a broad `button { all: unset }` reset — and if it does,
    // the ID selector alone isn't enough once the host escalates to
    // `!important` (all: unset !important collapses it to a tiny 26x26
    // inline blob with no background). Demonstrated, not theoretical — see
    // test/fixtures/real-world-host-important.html.
    + "#vc-bubble {\n"
    ...
    + "  background: #3d5fd1 !important;\n"
    + "  box-shadow: 0 10px 28px rgba(79, 127, 255, 0.4) !important;\n"
    ...
    + ".vc-header h4 { font-size: 0.98rem !important; font-weight: 700; margin: 0; color: #ffffff !important; text-transform: none !important; }\n"
    ...
    + ".vc-form { background: #182238 !important; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 16px; align-self: stretch; display: flex !important; flex-direction: column; gap: 10px; margin: 0; }\n"
    + ".vc-form label { font-size: 0.8rem; font-weight: 600; color: #eef1f8; }\n"
    // Same reasoning as #vc-bubble above: `input, textarea { all: unset
    // !important }` is a realistic (if uncommon) "nuclear reset" pattern —
    // demonstrated to strip padding/border/background entirely otherwise.
    ...
    + "  appearance: none;\n"
    + "}\n"
    + ".vc-form .vc-submit:hover { background: #33509e !important; }\n"
    ...
    + "  margin: 0 auto 14px;\n"
    + "}\n"
    + ".vc-thankyou h4 { font-size: 1rem !important; margin: 0 0 6px; color: #ffffff !important; text-transform: none !important; font-weight: 700; }\n"
  ...
  function initChat(el, formspreeAction) {
    ...
    function clearOptions() {
      // If the element the user just activated (e.g. a menu/back button) is
      // about to be removed along with its wrapper, removing a focused node
      // silently drops focus to <body> — which breaks the Escape handler and
      // Tab focus trap (both are bound to `win`, not document) until the
      // user manually clicks or tabs back in. Move focus to the dialog
      // container itself first, so keyboard/screen-reader users are never
      // silently ejected from the widget mid-conversation.
      var active = document.activeElement;
      var losingFocus = active && win.contains(active) && active !== win;
      body.querySelectorAll(".vc-options, .vc-form, .vc-thankyou").forEach(function (n) {
        if (losingFocus && n.contains(active)) win.focus();
        n.remove();
      });
    }
