...
A self-hosted chat widget — no third-party service, no volume limits, no fees. Copy the block below into any HTML page and it works standalone.

`index.html` in this repo is just a demo page showing the widget in context — it's not part of what you need to copy.
...
Paste this whole block right before `</body>` on any page. It's fully self-contained — no extra CSS setup, no dependencies.
...
<style>
  ...
  .vc-faq-item.vc-open .vc-faq-q::after {
  ...
  .vc-form .vc-submit:hover {
    background: #7aa0ff;
  }

  .vc-form .vc-submit:disabled {
    opacity: 0.6;
    cursor: not-allowed;
  }
...
(function () {
  ...
  var FORMSPREE_ACTION = "https://formspree.io/f/xeaokykk"; // ← put your own Formspree endpoint here
...
**Before you paste**, change two things to make it yours:
- `FORMSPREE_ACTION` — create a free form at [formspree.io](https://formspree.io) and put your own endpoint here so the contact form emails you instead of the demo inbox.
- The welcome message, header title (`Business Solutions Hub`), and menu options — just plain text/strings inside the `<script>` block, edit freely.
...
> Entirely optional — the widget is free to use, modify, and deploy without any obligation (see [LICENSE](LICENSE)). ⭐ Star the repo if it was useful!
